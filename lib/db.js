...
let connect = function (config) {
    ...
    function processOpenDatabase(err, mongoClient) {
        if (err) {
            throw err;
        }
    ...
    if (config.mongodb.connectionString) {
        mongodb.MongoClient.connect(config.mongodb.connectionString, config.mongodb, processOpenDatabase);
    } else {
        ...
        if (Array.isArray(host)) {
            ...
            mongodb.MongoClient.connect(`mongodb://${host || '127.0.0.1'}:${port || '27017'}/gbase-test`, dbOptions, processOpenDatabase);
